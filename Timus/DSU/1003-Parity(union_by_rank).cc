...
void init(int n) {
    ...
    for (int i = 0; i < n + 10; i++) parents[i] = i, ranks[i] = 0;

}

int find(int x) {
    while(x != parents[x]) x = parents[x];
    return x; // path compression wasn't required when use union by rank. strange!
}
